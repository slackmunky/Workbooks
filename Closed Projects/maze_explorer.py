    ...
    def explore(self):
        ...
        while current_room != "Treasure Room":
            ...
            for connected_room, weight in node.edges.items():
                ...
                print("Enter {0} for {1}: {2} cost".format(key, connected_room,
                                                           weight))
            valid_choices = [room[0] for room in node.edges.keys()]
            print("\nYou have accumulate {0} cost.".format(path_total))
...
def build_graph():
    ...
    antechamber = Vertex("Antechamber")
    ...
    graph.add_edge(kings_room, antechamber, 1)
    graph.add_edge(grand_gallery, kings_room, 2)
    graph.add_edge(grand_gallery, antechamber, 2)
    graph.add_edge(treasure_room, antechamber, 6)
